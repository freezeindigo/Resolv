...
**Model:** Groq `llama-3.3-70b-versatile` (paper eval); see `src/config/model_config.py` for production config
...
#### Arbiter Agent
**Model:** Anthropic `claude-haiku-4-5-20251001` (paper eval, via `--paper-mode`); `claude-sonnet-4-20250514` (production default in `src/config/model_config.py`)
...
**Why an agent:** This is the integration step. The arbiter must reason about tradeoffs: "structural_seepage has only 35% likelihood but 10× cost weight = adjusted score of 3.5. pipe_failure has 60% likelihood but 1× cost weight = score of 0.6. Route structural despite lower raw probability." This is asymmetric expected-value reasoning that benefits from a capable model with structured output.
